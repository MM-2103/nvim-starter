...
|---|---|---|
| LSP attached | `:LspInfo` | Server listed as "attached" |
...
- `:LspInfo` shows "no client attached" — install the server in `:Mason` and
  reopen the file.
- Completion doesn't appear — make sure the file type is detected. Try `:set
  ...
  `:set ft=python`.
- Diagnostics don't show — check `:LspInfo` to confirm the server is running.
  Some servers need a few seconds to start.
